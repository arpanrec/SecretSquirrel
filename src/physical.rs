...
pub async fn establish_connection() -> libsql::Result<Connection> {
...
pub async fn read(conn: &Connection, key: &str) -> Option<String> {
    let mut rows = conn
        .query(
...
pub async fn write(conn: &Connection, key: &str, value: &str) -> () {
...
pub async fn delete(conn: &Connection, key: &str) -> () {
    let current_epoch_time: i64 = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_secs() as i64;
